...
import {
  Search, Bell, Settings, LogOut, LayoutDashboard, BarChart3,
  GitCompareArrows, FileText, Moon, Sun, Monitor, X,
} from "lucide-react";
...
export function Topbar() {
  ...
  const runSearch = useCallback(async (q: string) => {
    ...
    } catch { /* ignore API errors in search */ }
        ...
        <div className="h-full px-6 flex items-center gap-4">
          ...
          <div ref={searchRef} className="flex-1 max-w-md relative">
            ...
            <input
              ...
              onFocus={() => {
                setSearchOpen(true);
                if (!query.trim()) setResults(NAV_ITEMS.map(r => ({ kind: "route" as const, ...r })));
              }}
              onKeyDown={e => {
            ...
            {searchOpen && results.length > 0 && (
              <div className="absolute top-full mt-2 left-0 right-0 rounded-xl border border-border bg-card shadow-lg overflow-hidden z-50">
                {results.some(r => r.kind === "route") && (
                  <p className="px-3 pt-2.5 pb-1 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Navegação</p>
                )}
                {results.filter(r => r.kind === "route").map(r => (
                  <SearchRow key={r.to + r.label} result={r} onSelect={() => handleSelect(r.to)} />
                ))}
                {results.some(r => r.kind === "analysis") && (
                  <p className="px-3 pt-2.5 pb-1 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground border-t border-border mt-1">Análises recentes</p>
                )}
                {results.filter(r => r.kind === "analysis").map((r, i) => (
          ...
          <div className="ml-auto flex items-center gap-2">
            <IconBtn onClick={() => setBellOpen(true)} title="Notificações"><Bell size={18} /></IconBtn>
            <IconBtn onClick={() => setSettingsOpen(true)} title="Configurações"><Settings size={18} /></IconBtn>
            ...
            <IconBtn onClick={handleLogout} title="Sair"><LogOut size={18} /></IconBtn>
          </div>
        </div>
      </header>
      ...
      <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-5">
            ...
            <button type="button" onClick={() => { setSettingsOpen(false); handleLogout(); }}
      ...
      <Dialog open={bellOpen} onOpenChange={setBellOpen}>
        <DialogContent className="max-w-sm">
          ...
          <div className="py-6 text-center space-y-2">
            ...
            <p className="text-xs text-muted-foreground/70">Alertas sobre suas análises aparecerão aqui.</p>
          </div>
        </DialogContent>
      </Dialog>
    </>
...
function ThemeBtn({ icon: Icon, label, active }: { icon: typeof Sun; label: string; active?: boolean }) {
  return (
    <button type="button"
      className={`flex flex-col items-center gap-1.5 py-2.5 px-2 rounded-lg border text-xs font-medium transition ${
        active ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:bg-accent/50 hover:text-foreground"
      }`}>
